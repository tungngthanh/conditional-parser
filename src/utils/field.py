...
class ChartDiscourseField(Field):
    """
    Field dealing with discourse trees.

    This field receives sequences of splitting decisions in pre-order,
    and returns two tensors representing the splitting decisions and labels on each discourse node.

    Examples:
        >>> sequence = [(0, 24, 30, 'Attribution_NS'), (0, 12, 24,'Joint_NN'), (12, 16, 24,'Attribution_SN')]
        >>> splits, labels = field.transform([sequence])  # this example field is built from ptb
        >>> splits
        tensor([[0, 24, 30],
                [0, 12, 24],
                [12, 16, 24]])
    ...
    def build(self, dataset, min_freq=1):
        counter = Counter(label
                          for seq in getattr(dataset, self.name)
                          for i, k, j, label in self.preprocess(seq))
    ...
    def transform(self, sequences):
        ...
        for sequence in sequences:
            if len(sequence) > 0:
                ...
                split_chart = torch.zeros(3, 1, dtype=torch.long)
                label_chart = torch.zeros(1, dtype=torch.long)
                # label_chart = torch.tensor([-1], dtype=torch.long)

            splits.append(split_chart)
            labels.append(label_chart)

        return list(zip(splits, labels))
...
class SubwordFieldSPMRL(Field):
    ...
    def preprocess(self, sequence):
        ...
        Returns:
            A list of preprocessed sequence.
        """
        from src.utils.util_spmrl import preprocess_spmrl
        if self.fn is not None:
            sequence = self.fn(sequence)
        if self.tokenize is not None:
