...
class PointingConstituencyZhParser(Parser):

    NAME = 'pointing-constituency-zh'
    MODEL = PointingConstituencyZhModel
    ...
    def _evaluate(self, loader):
        ...
        for words, feats, trees, (spans, labels), parsingorders in loader:
            ...
            preds = [TreeZh.build(tree,
                           [(i, j, self.CHART.vocab.itos[label])
                            for i, j, label in pred])
                     for tree, pred in zip(trees, preds)]
            total_loss += loss.item()
            metric([TreeZh.factorize(tree, self.args.delete, self.args.equal) for tree in preds],
                   [TreeZh.factorize(tree, self.args.delete, self.args.equal) for tree in trees])
        total_loss /= len(loader)
    ...
    def _predict(self, loader):
        ...
        for words, feats, trees, (spans, labels), parsingorders in progress_bar(loader):
            ...
            preds['trees'].extend( [TreeZh.build(tree,
                           [(i, j, self.CHART.vocab.itos[label])
                            for i, j, label in pred])
    ...
    def build(cls, path, min_freq=2, fix_len=20, **kwargs):
        ...
        if args.feat in ('char', 'bert'):
            ...
            transform = TreeZh(WORD=WORD, POS=FEAT, TREE=TREE, CHART=CHART, PARSINGORDER=PARSINGORDER)
